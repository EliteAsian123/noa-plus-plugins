...
// LIBRARY: noise.js [MODIFIED]                            //
// Code by: Stefan Gustavson, Peter Eastman, Joseph Gentle //
//                                                         //
// This allows for perlin noise.                           //
//                                                         //
/////////////////////////////////////////////////////////////
...
NoaTerrainGen.prototype.genTerrain = function(id, data, x, y, z, blockIDs) {
	switch (this.genType) {
		...
		case "simplex":
			for (var x1 = 0; x1 < data.shape[0]; ++x1) {
				for (var z1 = 0; z1 < data.shape[2]; ++z1) {
					var random = Math.floor(noise.simplex2((x1 + x) / 100, (z1 + z) / 100) * 10);
					for (var y1 = 0; y1 < data.shape[1]; ++y1) {
						if (y1 + y === random + 1) {
							data.set(x1, y1, z1, blockIDs[0]);
